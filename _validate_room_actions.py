...
try:
    ...
    # Parte 2D: las tres escenas compartidas necesitan una invitación aceptada.
    joel.execute_script("houseBed.click()")
    result["lie_invite_available"] = wait_for(joel, "!houseBedInviteLie.disabled && !houseBedInviteLie.hidden")
    joel.execute_script("houseBedInviteLie.click()")
    result["lie_invite_received"] = wait_for(princesa, "!houseBedInvitation.hidden && houseBedInvitationText.textContent.includes('acueste')")
    result["lie_requires_acceptance"] = princesa.execute_script("return !document.querySelector('[data-avatar-for=princesa]').classList.contains('is-in-bed')")
    princesa.execute_script("houseBedInvitationAccept.click()")
    result["lie_invite_accepted"] = wait_for(joel, "document.querySelector('[data-avatar-for=princesa]').classList.contains('is-in-bed')")
    ...
    result["private_invite_received"] = wait_for(princesa, "!houseBedInvitation.hidden && houseBedInvitationText.textContent.includes('puerta')")
    result["private_requires_acceptance"] = joel.execute_script("return !houseBed.classList.contains('is-private-moment')")
    princesa.execute_script("houseBedInvitationAccept.click()")
    ...
    joel.execute_script("houseBedIntimate.click()")
    wait_for(princesa, "!houseBedInvitation.hidden")
    princesa.execute_script("houseBedInvitationAccept.click()")
    ...
    joel.execute_script("houseBedIntimate.click();houseBedInviteSleep.click()")
    result["sleep_invite_received"] = wait_for(princesa, "!houseBedInvitation.hidden && houseBedInvitationText.textContent.includes('mimir')")
    princesa.execute_script("houseBedInvitationAccept.click()")
    result["sleep_cuddled_shared"] = wait_for(joel, "document.querySelectorAll('.house-avatar.is-sleeping').length === 2") and wait_for(princesa, "document.querySelectorAll('.house-avatar.is-sleeping').length === 2")
